...
const previewPlatforms = ["LinkedIn", "Instagram", "X / Twitter"];
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="rounded-lg border border-white/[0.09] bg-white/[0.025] p-3.5">
                  ...
                  <div className="mt-3 space-y-2">
                    {platforms.slice(0, 3).map((platform, index) => (<div key={platform.label} className="flex items-center gap-2 rounded-md bg-white/[0.04] p-2"><span className={`grid size-5 place-items-center rounded text-[9px] font-bold ${platform.className}`}>{platform.label}</span><span className="font-mono text-[10px] text-white/75">{previewPlatforms[index]}</span><CircleCheck className="ml-auto size-3.5 text-emerald-400" /></div>))}
                  </div>
                </div>
              </div>
